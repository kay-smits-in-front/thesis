"""
RNN Optimization with Early Pruning
...
class RNNWithPhysics(keras.Model):
	def __init__(self, architecture, dropout_rate, physics_weight, scaler_X, scaler_y, column_indices):
		...
		for i, units in enumerate(architecture):
			self.rnn_layers.append(SimpleRNN(units, return_sequences=(i < len(architecture)-1)))
...
def train_single_model(splits, scaler_X, scaler_y, column_indices, architecture, batch_size, physics_weight, config):
	model = RNNWithPhysics(architecture, config['dropout_rate'], physics_weight, scaler_X, scaler_y, column_indices)
