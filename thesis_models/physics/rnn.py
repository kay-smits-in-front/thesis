...
class RNN_PINN(keras.Model):
	def __init__(self, rnn_config):
		...
		self.rnn2 = SimpleRNN(rnn_config['layer2_units'], return_sequences=True)
		self.dropout2 = Dropout(rnn_config['dropout_rate'])
		self.rnn3 = SimpleRNN(rnn_config['layer3_units'], return_sequences=False)
		self.dropout3 = Dropout(rnn_config['dropout_rate'])
...
def train_single_model(splits, scaler_X, scaler_y, column_mapping, physics_weight, dataset_name, config):
	model_name = f"RNN_PINN_pw{physics_weight}_{dataset_name}"

	tracker = CarbonTracker(epochs=1)
	tracker.epoch_start()

	model = RNN_PINN(config['rnn_config'])
...
if __name__ == "__main__":
	...
	if os.path.exists(regular_path) and os.path.exists(weather_path):
		...
		train_models(speed_trials_regular, "Regular", target_col, CONFIG)
		train_models(speed_trials_weather, "Weather", target_col, CONFIG)
	else:
