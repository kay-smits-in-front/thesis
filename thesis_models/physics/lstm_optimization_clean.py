"""
LSTM Optimization - CLEAN VERSION
...
class LSTMModel(keras.Model):
	"""Simple LSTM model for data-only training (no physics)"""
...
def prepare_data(data, target_col, timesteps):
	...
	# CRITICAL: Split BEFORE scaling (60/20/20)
...
def run_optimization(data, target_col, config):
	...
	print(f"\n{'='*70}")
	print(f"LSTM OPTIMIZATION - DATA-ONLY MODEL (physics_weight=0.0)")
...
if __name__ == "__main__":
	...
	if os.path.exists(regular_path):
		...
		output_file = f'{CONFIG["output_dir"]}/lstm_optimization_clean_{timestamp}.json'
