...
class MLPWithPhysics(keras.Model):
	def __init__(self, input_dim, n_lags, feature_cols, column_indices, scaler_X, scaler_y, physics_weight, architecture):
		super().__init__()
	...
	def compute_physics_loss(self, inputs, predictions):
		...
		u_scaled = inputs_reshaped[:, -1, self.u_idx % self.n_features]
		v_scaled = inputs_reshaped[:, -1, self.v_idx % self.n_features]
		r_scaled = inputs_reshaped[:, -1, self.r_idx % self.n_features]
		nP_scaled = inputs_reshaped[:, -1, self.nP_idx % self.n_features]

		u = u_scaled * self.scaler_X_std[self.u_idx % self.n_features] + self.scaler_X_mean[self.u_idx % self.n_features]
		v = v_scaled * self.scaler_X_std[self.v_idx % self.n_features] + self.scaler_X_mean[self.v_idx % self.n_features]
		r = r_scaled * self.scaler_X_std[self.r_idx % self.n_features] + self.scaler_X_mean[self.r_idx % self.n_features]
		nP = nP_scaled * self.scaler_X_std[self.nP_idx % self.n_features] + self.scaler_X_mean[self.nP_idx % self.n_features]
...
def prepare_data(df, target_col, n_lags):
	...
	return splits, scaler_X, scaler_y, feature_cols, (u_idx, v_idx, r_idx, nP_idx)


def train_single_model(splits, scaler_X, scaler_y, feature_cols, column_indices, architecture, batch_size, physics_weight, config):
	input_dim = splits['X_train'].shape[1]

	model = MLPWithPhysics(input_dim, config['n_lags'], feature_cols, column_indices, scaler_X, scaler_y, physics_weight, architecture)
...
def run_optimization(data, dataset_name, target_col, config):
	...
	splits, scaler_X, scaler_y, feature_cols, column_indices = prepare_data(data, target_col, config['n_lags'])
	...
	for arch in config['architectures']:
		for batch_size in config['batch_sizes']:
			for physics_weight in config['physics_weights']:
				...
				result = train_single_model(splits, scaler_X, scaler_y, feature_cols, column_indices,
