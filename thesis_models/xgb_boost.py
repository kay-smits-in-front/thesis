...
def create_multivariate_lag_features(df, target_col, n_lags, forecast_horizon):
	# Define columns to exclude to prevent feature leakage
...
def train_and_evaluate(dataset, dataset_name, n_lags=60):
	...
	model_name = f"XGBoost_lags{n_lags}_{dataset_name.replace(' ', '_')}"
	...
	print(f"\nXGBoost with n_lags={n_lags} - {dataset_name}:")
...
# Use n_lags=30 instead of 60 to reduce overfitting (fewer features)
# Change back to 60 if you want more features
